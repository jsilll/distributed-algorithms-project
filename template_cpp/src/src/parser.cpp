...
std::string Parser::Host::ipReadable() const
{
    in_addr tmp_ip;
    tmp_ip.s_addr = ip;
...
unsigned short Parser::Host::portReadable() const { return ntohs(port); }

bool Parser::Host::isValidIpAddress(const char *ipAddress)
{
...
Parser::Parser(const int argc, char const *const *argv, bool withConfig)
...
const char *Parser::hostsPath() const
{
    checkParsed();
    return hostsPath_.c_str();
}

const char *Parser::outputPath() const
{
    checkParsed();
    return outputPath_.c_str();
}

const char *Parser::configPath() const
{
    checkParsed();
    if (!withConfig)
    {
...
std::vector<Parser::Host> Parser::hosts()
{
